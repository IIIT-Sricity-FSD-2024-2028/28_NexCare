...
function initDoctorInfo() {
    const rawUser = sessionStorage.getItem('nexcare_user_data');
    if (rawUser) {
        try {
            doctorProfile = JSON.parse(rawUser);
            document.getElementById('doctorName').textContent = doctorProfile.name || 'Doctor';
            document.getElementById('doctorDept').textContent = doctorProfile.dept || 'Consultant Doctor';
...
async function loadAppointments() {
    ...
    try {
        ...
        if (response && response.success && Array.isArray(response.data)) {
            ...
            tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">No appointments found.</td></tr>';
        }
    } catch (err) {
        console.error('Failed to load appointments:', err);
        tbody.innerHTML = '<tr><td colspan="7" class="loading-cell" style="color:#ef4444;">Failed to load appointments. Make sure backend is running.</td></tr>';
    }
}
...
function renderAppointments(list) {
    ...
    if (!list || list.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">No matching appointments found.</td></tr>';
        return;
    }

    tbody.innerHTML = list.map(apt => {
        const dateStr = apt.appointmentDate ? new Date(apt.appointmentDate).toLocaleString() : (apt.date || 'N/A');
        ...
        return `
            <tr>
                <td><strong>${apt.emrToken || apt.id}</strong></td>
                <td>${apt.patientName || apt.patientId || 'Patient'}</td>
